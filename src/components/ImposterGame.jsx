...
export default function ImposterGame({ playerNames, onEndGame }) {
  ...
  const startGame = () => {
    const data = getRandomImposterWord();
    setSecretData(data);

    let assignedRoles = playerNames.map(name => ({
      ...
      <div className="glass-panel p-6 max-w-md w-full relative z-10 flex flex-col h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2 mb-6">
          {activePlayers.map((p, index) => (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-surface border border-white/10 flex items-center justify-center mr-4 shadow-inner">
                <span className="text-gray-300 font-black text-sm">{p.name.substring(0, 2).toUpperCase()}</span>
              </div>
  ...
  if (phase === 'end') {
    return (
      <div className="glass-panel p-8 max-w-md w-full text-center relative z-10 h-[70vh] flex flex-col justify-center">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-8 text-left max-h-48 overflow-y-auto">
          ...
          {players.map(p => (
            <div key={p.id} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
              <span className={`font-bold ${!p.isAlive ? 'line-through text-gray-500' : 'text-white'}`}>{p.name}</span>
